<?php

declare(strict_types=1);

return [
    'kind' => [
        'tutorial' => 'Tutoriál',
        'how_to' => 'Návod',
        'reference' => 'Reference',
        'explanation' => 'Souvislosti',
    ],

    // Shown under the title so a reader knows what kind of page they opened
    // before they invest three minutes in the wrong one.
    'promise' => [
        'tutorial' => 'Projdi to od začátku do konce — na konci ti to bude fungovat.',
        'how_to' => 'Postup k jednomu konkrétnímu cíli. Předpokládá, že víš proč.',
        'reference' => 'K nahlédnutí: hodnoty, volby, limity. Bez vyprávění.',
        'explanation' => 'Proč to funguje takhle. Čti mimo klávesnici.',
    ],

    'format' => [
        'markdown' => 'Markdown',
        'rich-text' => 'Textový',
        'blocks' => 'Bloky',
    ],

    'status' => [
        'draft' => 'Koncept',
        'in_review' => 'Ke kontrole',
        'published' => 'Publikováno',
        'archived' => 'Archivováno',
    ],

    'visibility' => [
        'internal' => 'Interní',
        'public' => 'Veřejné',
    ],

    'home' => [
        'title' => 'Znalostní báze',
        'lead' => 'Návody, postupy a reference ke všemu, co tu běží.',
        'placeholder' => 'Co potřebuješ udělat?',
        'popular' => 'Nejčtenější',
        'empty' => 'Zatím tu nic není.',
    ],

    'search' => [
        'results' => '{1} :count výsledek|[2,4] :count výsledky|[5,*] :count výsledků',
        'none' => 'Na „:term“ nic nesedí.',
        'none_hint' => 'Zkus méně slov, nebo projdi kategorie.',
        'open' => 'Hledat',
        'hint' => 'Piš pro hledání, ↑↓ pohyb, ↵ otevřít.',
    ],

    'article' => [
        'edit' => 'Upravit',
        'toc' => 'Na téhle stránce',
        'reading_time' => 'čtení :minutes min',
        'updated' => 'Upraveno :date',
        'reviewed' => 'Ověřeno :date',
        'stale' => 'Tuhle stránku už delší dobu nikdo neověřil — může být zastaralá.',
        'related' => 'Číst dál',
        'back' => 'Zpět do znalostní báze',
    ],

    'feedback' => [
        'question' => 'Pomohlo to?',
        'yes' => 'Ano',
        'no' => 'Ne',
        'thanks' => 'Díky, zapsáno.',
        'why' => 'Co chybělo?',
        'why_placeholder' => 'Krok, který nefungoval, věc, kterou jsi nenašel…',
        'send' => 'Odeslat',
        'sent' => 'Díky. Právě z tohohle se píše další verze.',
    ],

    'editor' => [
        'task_list' => 'Zaškrtávací seznam',
        'subscript' => 'Dolní index',
        'superscript' => 'Horní index',
        'clear_format' => 'Vymazat formátování',
        'horizontal_rule' => 'Vodorovný předěl',
        'style_paragraph' => 'Normální text',
        'style_h2' => 'Nadpis 2',
        'style_h3' => 'Nadpis 3',
        'style_quote' => 'Citace',
        'style_code' => 'Blok kódu',
        'font_family' => 'Písmo',
        'font_default' => 'Výchozí',
        'font_sans' => 'Bezpatkové',
        'font_serif' => 'Patkové',
        'font_mono' => 'Strojopis',
        'invisible' => 'Skryté znaky',
        'placeholder' => 'Piš…',
        'selection_tools' => 'Úpravy označeného textu',
        'insert' => 'Vložit',
        'align_left' => 'Vlevo',
        'align_center' => 'Na střed',
        'align_right' => 'Vpravo',
        'align_justify' => 'Do bloku',
        'font_size' => 'Velikost',
        'size_default' => 'Základní',
        'size_small' => 'Menší',
        'size_large' => 'Větší',
        'size_huge' => 'Největší',
        'text_color' => 'Barva písma',
        'text_background' => 'Podbarvení',
        'color_default' => 'Základní',
        'color_red' => 'Červená',
        'color_orange' => 'Oranžová',
        'color_green' => 'Zelená',
        'color_blue' => 'Modrá',
        'color_purple' => 'Fialová',
        'color_gray' => 'Šedá',
        'color_yellow' => 'Žlutá',
        'indent' => 'Odsadit',
        'outdent' => 'Zrušit odsazení',
        'row_before' => 'Řádek nad',
        'row_after' => 'Řádek pod',
        'row_delete' => 'Smazat řádek',
        'column_before' => 'Sloupec vlevo',
        'column_after' => 'Sloupec vpravo',
        'column_delete' => 'Smazat sloupec',
        'header_row' => 'Hlavičkový řádek',
        'merge_cells' => 'Sloučit/rozdělit',
        'table_delete' => 'Smazat tabulku',
        'drag_hint' => 'Přetažením změň pořadí',
        'row' => 'řádek',
        'column' => 'sloupec',
        'tone_info' => 'Poznámka',
        'tone_success' => 'Tip',
        'tone_warning' => 'Pozor',
        'tone_danger' => 'Nebezpečí',
        'add_block' => 'Přidat blok',
        'pick_block' => 'Jaký blok?',
        'block_search' => 'Hledat typ…',
        'tone' => 'Tón',
        'level' => 'Úroveň',
        'term' => 'Pojem',
        'meaning' => 'Význam',
        'question' => 'Otázka',
        'answer' => 'Odpověď',
        'items_hint' => 'Jedna položka na řádek',
        'table_hint' => 'Sloupce oddělené |, první řádek je hlavička',
        'video_unsupported' => 'Vložení z tohohle zdroje není povolené.',
        'image_title' => 'Vložit obrázek',
        'drop_here' => 'Přetáhni sem obrázek',
        'or_click' => 'nebo klikni a vyber soubor',
        'uploading' => 'Nahrává se…',
        'gallery' => 'Už nahrané',
        'limits' => ':mimes · do :size MB',
        'pick_image' => 'Vybrat…',
        'duplicate' => 'Duplikovat',
        'list' => 'seznam',
        'image' => 'obrázek',
        'table' => 'tabulka',
        'undo' => 'Zpět',
        'redo' => 'Znovu',
        'link_prompt' => 'Adresa odkazu (prázdné ho odebere):',
        'image_prompt' => 'Adresa obrázku:',
        'move_up' => 'Nahoru',
        'move_down' => 'Dolů',
        'block' => [
            'callout_title' => 'Titulek (volitelný)',
            'code_language' => 'Jazyk kódu',
            'no_language' => 'Bez zvýraznění',
            'code_title' => 'Název souboru (volitelný)',
            'line_numbers' => 'Čísla řádků',
            'diff_indicators' => 'Značky +/− u diffu',
            'code_annotations' => 'Anotace přímo v kódu: [tl! ++] přidáno, [tl! --] odebráno, [tl! focus] zvýraznit, [tl! collapse] sbalit.',
            'alt' => 'Popis obrázku',
            'caption' => 'Popisek',
            'unknown' => 'Blok typu „:type“ — v téhle instalaci pro něj není editor.',
            'empty' => 'Prázdné. Přidej první blok níž.',
            'heading' => 'Nadpis',
            'text' => 'Text',
            'list' => 'Seznam',
            'steps' => 'Kroky',
            'quote' => 'Citace',
            'divider' => 'Předěl',
            'callout' => 'Upozornění',
            'definition' => 'Pojem',
            'checklist' => 'Kontrolní seznam',
            'code' => 'Kód',
            'terminal' => 'Příkaz',
            'table' => 'Tabulka',
            'image' => 'Obrázek',
            'video' => 'Video',
            'file' => 'Soubor',
            'article' => 'Odkaz na článek',
            'faq' => 'Otázka',
        ],
        'block_hint' => [
            'heading' => 'Rozdělí stránku a objeví se v obsahu',
            'text' => 'Odstavec',
            'list' => 'Odrážky',
            'steps' => 'Číslovaný postup',
            'quote' => 'Doslovné znění',
            'divider' => 'Vodorovná čára',
            'callout' => 'Co se nesmí přehlédnout',
            'definition' => 'Termín a jeho význam',
            'checklist' => 'Co odškrtat před dalším krokem',
            'code' => 'Ukázka se zvýrazněním',
            'terminal' => 'Co napsat do terminálu',
            'table' => 'Řádky oddělené novým řádkem, sloupce svislítkem',
            'image' => 'Snímek nebo schéma',
            'video' => 'YouTube nebo Vimeo',
            'file' => 'Odkaz ke stažení',
            'article' => 'Vede jinam do báze',
            'faq' => 'Rozbalovací dotaz a odpověď',
        ],
        'block_group' => [
            'text' => 'Text',
            'highlight' => 'Zvýraznění',
            'code' => 'Technické',
            'media' => 'Média',
            'links' => 'Odkazy',
        ],
        'link' => 'odkaz',
        'markdown_hint' => 'Markdown · ⌘B ⌘I ⌘K',
    ],

    'admin' => [
        'category' => 'Kategorie',
        'status' => 'Stav',
        'slug_warning' => 'Změna adresy rozbije odkazy, které už někdo sdílel.',
        'excerpt' => 'Shrnutí',
        'excerpt_hint' => 'Ukáže se ve výsledcích hledání a ve výpisech.',
        'visibility_label' => 'Vidí',
        'categories' => 'Kategorie',
        'new_category' => 'Nová kategorie',
        'category_name' => 'Název',
        'order' => 'Pořadí',
        'cancel' => 'Zrušit',
        'delete' => 'Smazat',
        'article_count' => '{0} bez článků|{1} :count článek|[2,4] :count články|[5,*] :count článků',
        'delete_category_confirm' => 'Smazat kategorii? Články zůstanou a půjde je zařadit jinam.',
        'needs_work' => 'Potřebuje pozornost',
        'all' => 'Všechny články',
        'stale' => 'Dlouho neověřené',
        'unhelpful' => 'Špatně hodnocené',
        'summary_attention' => '{0} nic nehnije|{1} :count potřebuje pozornost|[2,4] :count potřebují pozornost|[5,*] :count potřebuje pozornost',
        'clear_reason' => 'Zrušit filtr',
        'column_article' => 'Článek',
        'column_reason' => 'Proč',
        'column_reviewed' => 'Ověřeno',
        'reason_draft' => 'Koncept',
        'reason_stale' => 'Neověřeno :since',
        'reason_stale_never' => 'Nikdy neověřeno',
        'reason_unhelpful' => '{1} :score % · :count hlas|[2,4] :score % · :count hlasy|[5,*] :score % · :count hlasů',
        'filter_status' => 'Každý stav',
        'filter_category' => 'Každá kategorie',
        'new' => 'Nový článek',
        'save' => 'Uložit',
        'saved' => 'Uloženo',
        'preview' => 'Náhled',
        'write' => 'Psát',
        'mark_reviewed' => 'Pořád platí',
        'note' => 'Co se změnilo?',
        'note_placeholder' => 'Uloží se k revizi, čtenářům se nezobrazí.',
        'empty' => 'Není co opravovat. Báze je v pořádku.',
    ],
];
